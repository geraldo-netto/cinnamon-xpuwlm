...
const PlatformGuidance = require("./platform-guidance.js");
...
const RUNTIME_RECOVERY = PlatformGuidance.POSIX_RUNTIME_RECOVERY;
...
function recoveryModel(state, guidancePort = PlatformGuidance.POSIX_GUIDANCE) {
    const health = healthOf(state);
    const guidance = PlatformGuidance.requireGuidancePort(guidancePort)
        .recoveryFor(health.runtime);
...
function toViewModel(state, nowMs = Date.now(), guidance) {
    ...
    return {
        ...
        recovery: recoveryModel(state, guidance),
