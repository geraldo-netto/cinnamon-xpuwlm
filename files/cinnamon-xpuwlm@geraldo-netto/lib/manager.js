...
class WorkloadManager {
    constructor({
        ...
        this._workloadRuntime = new WorkloadRuntimeController({
            jobSubmitter,
            inputCatalog,
            workloadRegistry: this._workloadRegistry,
            errorReporter: this._errors,
            clock: this._clock,
            logger: this._logger,
            scheduler: this._scheduler,
            publish: () => this._publish(),
            inputRoots: () => this._inputRoots(),
            controlAvailable: () => this._controlServiceAvailable,
            managerDisposed: () => this._disposed,
            fallbackFailureText: controlFailureText,
            serviceStoppedText: _(SERVICE_STOPPED_TEXT),
        });
    }
    ...
    _acceptSnapshot(sequence, snapshot, recovered = true) {
        ...
        this._workloadRuntime.acceptSnapshot();
        if (recovered) {
    ...
    refreshInputs() {
        this._ensureActive();
        return this._workloadRuntime.refreshInputs();
    }
    ...
    jobBlocker(profileId) {
        return this._workloadRuntime.jobBlocker(profileId);
    }

    submitJob(profileId, picture) {
        this._ensureActive();
        return this._workloadRuntime.submitJob(profileId, picture);
    }
    ...
    state() {
        ...
        const runtime = this._workloadRuntime.state();
    ...
    dispose() {
        if (this._disposed) {
            return this._workloadRuntime.dispose();
        }
        this._disposed = true;
        this._cancelExpiry();
        this._cancelRuntimeRead();
