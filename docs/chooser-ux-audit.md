...
| --- | --- |
| Aesthetic-Usability Effect | Use Cinnamon's themed native GTK chooser; do not introduce a visually foreign custom picker. |
...
| Flow | Preserve keyboard navigation, give the chooser exclusive visible focus, and restore the workflow popup when selection finishes. |
...
| Occam's Razor | Reuse one chooser lifecycle and the platform dialog instead of adding a second picker framework or background watcher. |
...
| Selective Attention | Never leave the applet popup competing with the modal chooser; move focus to the chooser and back to the resulting state. |
...
Automated coverage checks the native modal/present sequence, popup focus
handoff and restoration, one-shot response delivery, reload teardown, keyboard
